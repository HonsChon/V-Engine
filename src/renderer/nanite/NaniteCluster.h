...
/**
 * NaniteCluster.h - Nanite 风格的Mesh Cluster 数据结构
 * 
 * Nanite 的核心思想是将大型网格分割成小的Cluster（簇），
 * 每个 Cluster 包含约64-128 个三角形，便于：
 * 1. GPU 上进行细粒度剔除
...
// Cluster Group 中的最大Cluster 数量（用于LOD 层级）
constexpr uint32_t CLUSTER_GROUP_MAX_SIZE = 8;

// ============== 数据结构 ==============

/**
 * GPU 端的 Cluster 数据（用于Compute Shader 剔除）
 * 必须和shader 中的结构匹配
 * 注意：必须先定义，因为Cluster 结构体会引用它
 */
struct GPUClusterData {
    ...
    // 层级信息（用于DAG 遍历）
    uint32_t parentGroupIndex;  // 父Group 索引
    uint32_t flags;             // 标志位 bit0=enabled, bit1=isLeaf
...
struct ClusterBounds {
    ...
    float lodError;             // LOD 误差（用于LOD 选择）
    
    glm::vec3 aabbMax;          // AABB 最大点
    float screenSizeThreshold;  // 屏幕尺寸阈值
    
    // 法线锥（用于背面剔除优化）
    glm::vec3 coneAxis;         // 法线锥轴向
    float coneAngleCos;         // 法线锥角度的余弦（用于cone culling）
};

/**
 * Cluster - Nanite 的基本渲染单元
 * 
...
struct Cluster {
    ...
    // 顶点数据
    std::vector<Vertex> vertices;       // 未压缩顶点
    ...
    uint32_t parentGroupIndex = ~0u;    // 所属Cluster Group 索引
    uint32_t clusterGroupIndex = 0;     // 所属Cluster Group 索引
    ...
    // GPU 端的 Cluster 数据（用于Compute Shader）
 ...
 * 
 * 用于构建 LOD 层级）
 * - 多个相邻的Cluster 组成一个Group
 * - 简化后生成更粗糙的 Cluster 作为父级
 */
struct ClusterGroup {
    std::vector<uint32_t> clusterIndices;  // 含Cluster 索引
    uint32_t parentClusterIndex = ~0u;     // 父（简化后的）Cluster 索引
    
    ClusterBounds bounds;                  // 整个 Group 的包围盒
    float maxChildError = 0.0f;            // 所属Cluster 的最大误差
    
    uint32_t lodLevel = 0;                 // 所有LOD 层级
};

/**
 * ClusterizedMesh - 经过 Cluster 划分的网格
 * 
 * 包含：
 * - 所有LOD 层级的Clusters
 * - Cluster Groups（层级结构）
...
public:
    ...
    // GPU 数据
    uint32_t gpuClusterDataOffset = 0;  // 在全局 Cluster 缓冲区中的偏移
