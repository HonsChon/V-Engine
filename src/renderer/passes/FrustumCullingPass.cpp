...
void FrustumCullingPass::createBuffers(uint32_t maxInstCount) {
    ...
    // Visible Indices Buffer: 存储通过剔除测试的实例索�?
    ...
    // 每个网格一�?DrawIndexedIndirectCommand
    ...
    // Visible Indices Readback Buffer: CPU 可读的可见索�?
...
void FrustumCullingPass::updateInstances(const std::vector<GPUInstanceData>& instances) {
    ...
    if (currentInstanceCount > maxInstances) {
        // 需要重新分配更大的缓冲�?
        cleanup();
        createBuffers(currentInstanceCount * 2); // 预留一些空�?
        createDescriptorSet();
    }
    
    // 上传实例数据（这里简化处理，实际应使�?staging buffer�?
