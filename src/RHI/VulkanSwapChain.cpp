...
void VulkanSwapChain::createSwapChain() {
    ...
    VkExtent2D extent = chooseSwapExtent(swapChainSupport.capabilities);                    // 分辨�?
    ...
    createInfo.surface = device->getSurface();           // 绑定的窗口表�?
    ...
    createInfo.imageExtent = extent;                     // 图像分辨�?
    createInfo.imageArrayLayers = 1;                     // 图像层数（非VR�?�?
    ...
    if (indices.graphicsFamily != indices.presentFamily) {
        ...
        // 图形队列和呈现队列相同：使用独占模式，性能最�?
        createInfo.imageSharingMode = VK_SHARING_MODE_EXCLUSIVE;
    }

    createInfo.preTransform = swapChainSupport.capabilities.currentTransform;  // 保持当前变换
    createInfo.compositeAlpha = VK_COMPOSITE_ALPHA_OPAQUE_BIT_KHR;  // 不透明，不与其他窗口混�?
 ...
 * 类比：VkImage 是一块内存，VkImageView �?如何解读这块内存"的说明书�?
 */
void VulkanSwapChain::createImageViews() {
    ...
    for (size_t i = 0; i < swapChainImages.size(); i++) {
        ...
        createInfo.image = swapChainImages[i];           // 关联的图�?
        ...
        createInfo.subresourceRange.baseMipLevel = 0;    // 第一�?mipmap 级别
 ...
 * 它不执行实际渲染，而是描述"渲染会用到什么、会产生什�?的蓝图�?
 * 
 * 本函数创建一个包含颜色附件和深度附件的渲染通道�?
 * - 颜色附件：存储渲染结果，最终呈现到屏幕
 * - 深度附件：存储深度值，用于正确�?D遮挡关系
 */
void VulkanSwapChain::createRenderPass() {
    ...
    colorAttachment.loadOp = VK_ATTACHMENT_LOAD_OP_CLEAR;   // 渲染前：清除为指定颜�?
    ...
    colorAttachment.finalLayout = VK_IMAGE_LAYOUT_PRESENT_SRC_KHR;     // 渲染后布局：准备呈�?
    ...
    depthAttachment.loadOp = VK_ATTACHMENT_LOAD_OP_CLEAR;    // 渲染前：清除为最远深�?
    depthAttachment.storeOp = VK_ATTACHMENT_STORE_OP_DONT_CARE;  // 渲染后：不需要保�?
    ...
    colorAttachmentRef.attachment = 0;   // 引用附件数组中的�?0 个（颜色附件�?
    colorAttachmentRef.layout = VK_IMAGE_LAYOUT_COLOR_ATTACHMENT_OPTIMAL;  // 子通道中的布局

    VkAttachmentReference depthAttachmentRef{};
    depthAttachmentRef.attachment = 1;   // 引用附件数组中的�?1 个（深度附件�?
    depthAttachmentRef.layout = VK_IMAGE_LAYOUT_DEPTH_STENCIL_ATTACHMENT_OPTIMAL;

    // ─────────────────────────────────────────────────────────────────────────
    // 子通道描述
    // ─────────────────────────────────────────────────────────────────────────
    // 一个渲染通道可以包含多个子通道（用于多通道渲染），这里只用一�?
    VkSubpassDescription subpass{};
    subpass.pipelineBindPoint = VK_PIPELINE_BIND_POINT_GRAPHICS;  // 绑定到图形管�?
    ...
    dependency.srcSubpass = VK_SUBPASS_EXTERNAL;  // 依赖来源：外部操�?
 ...
 * 实现正确�?3D 遮挡关系（近的物体遮挡远的物体）�?
 * 
 * 创建内容�?
 * - VkImage: 存储深度数据的图�?
 * - VkDeviceMemory: 图像使用�?GPU 内存
 * - VkImageView: 访问深度图像的视�?
 */
void VulkanSwapChain::createDepthResources() {
    // 查找硬件支持的深度格式（�?D32_SFLOAT、D32_SFLOAT_S8_UINT、D24_UNORM_S8_UINT�?
    VkFormat depthFormat = device->findDepthFormat();

    // 创建深度图像和分配内�?
    // - TILING_OPTIMAL: GPU 优化布局，性能最�?
    // - USAGE_DEPTH_STENCIL_ATTACHMENT_BIT: 用作深度/模板附件
    // - DEVICE_LOCAL_BIT: 存储�?GPU 专用内存�?
...
void VulkanSwapChain::createFramebuffers() {
    ...
    for (size_t i = 0; i < swapChainImageViews.size(); i++) {
        ...
        std::array<VkImageView, 2> attachments = {
            swapChainImageViews[i],  // 颜色附件：每帧不�?
        ...
        framebufferInfo.width = swapChainExtent.width;   // 帧缓冲宽�?
        framebufferInfo.height = swapChainExtent.height; // 帧缓冲高�?
        framebufferInfo.layers = 1;                      // 层数（非数组纹理�?�?
...
/**
 * @brief 清理所有交换链相关�?Vulkan 资源
 * 
 * 销毁顺序是创建顺序的逆序（依赖关系）�?
 * 帧缓�?�?深度资源 �?渲染通道 �?图像视图 �?交换�?
 * 
 * 注意：交换链图像（swapChainImages）由交换链自动管理，不需要手动销�?
 */
...
/**
 * @brief 选择最佳的表面格式（颜色格�?+ 颜色空间�?
 * @param availableFormats 硬件支持的格式列�?
 * @return 选中的表面格�?
 * 
 * 优先选择：B8G8R8A8_SRGB + SRGB_NONLINEAR
 * - B8G8R8A8: 每通道8位，总共32位颜�?
 * - SRGB: 标准 RGB 颜色空间，颜色更准确，符合人眼感�?
 */
...
/**
 * @brief 选择最佳的呈现模式
 * @param availablePresentModes 硬件支持的呈现模式列�?
 * @return 选中的呈现模�?
 * 
 * 呈现模式说明�?
 * - IMMEDIATE: 立即提交，可能导致画面撕�?
 * - FIFO: 先进先出（垂直同步），无撕裂但可能有延迟
 * - FIFO_RELAXED: 类似 FIFO，但队列为空时立即显�?
...
VkExtent2D VulkanSwapChain::chooseSwapExtent(const VkSurfaceCapabilitiesKHR& capabilities) {
    ...
    if (capabilities.currentExtent.width != std::numeric_limits<uint32_t>::max()) {
        ...
        // 限制宽度�?[minWidth, maxWidth] 范围�?
        actualExtent.width = std::clamp(actualExtent.width, 
                                       capabilities.minImageExtent.width, 
                                       capabilities.maxImageExtent.width);
        // 限制高度�?[minHeight, maxHeight] 范围�?
