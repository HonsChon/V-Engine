...
void VulkanSwapChain::createSwapChain() {
    ...
    createInfo.imageArrayLayers = 1;                     // 图像层数（非VR一：
 ...
 * 类比：VkImage 是一块内存，VkImageView 是如何解读这块内存"的说明书。
 */
void VulkanSwapChain::createImageViews() {
    ...
    for (size_t i = 0; i < swapChainImages.size(); i++) {
        ...
        createInfo.subresourceRange.baseMipLevel = 0;    // 第一一mipmap 级别
 ...
 * 它不执行实际渲染，而是描述"渲染会用到什么、会产生什么的蓝图。
 * 
 * 本函数创建一个包含颜色附件和深度附件的渲染通道：
 * - 颜色附件：存储渲染结果，最终呈现到屏幕
 * - 深度附件：存储深度值，用于正确的D遮挡关系
 */
void VulkanSwapChain::createRenderPass() {
    ...
    colorAttachmentRef.attachment = 0;   // 引用附件数组中的第0 个（颜色附件：
    colorAttachmentRef.layout = VK_IMAGE_LAYOUT_COLOR_ATTACHMENT_OPTIMAL;  // 子通道中的布局

    VkAttachmentReference depthAttachmentRef{};
    depthAttachmentRef.attachment = 1;   // 引用附件数组中的第1 个（深度附件：
    depthAttachmentRef.layout = VK_IMAGE_LAYOUT_DEPTH_STENCIL_ATTACHMENT_OPTIMAL;

    // ─────────────────────────────────────────────────────────────────────────
    // 子通道描述
    // ─────────────────────────────────────────────────────────────────────────
    // 一个渲染通道可以包含多个子通道（用于多通道渲染），这里只用一与
 ...
 * 实现正确的3D 遮挡关系（近的物体遮挡远的物体）。
 * 
 * 创建内容：
 * - VkImage: 存储深度数据的图像
 * - VkDeviceMemory: 图像使用的GPU 内存
 * - VkImageView: 访问深度图像的视图
 */
void VulkanSwapChain::createDepthResources() {
    // 查找硬件支持的深度格式（如D32_SFLOAT、D32_SFLOAT_S8_UINT、D24_UNORM_S8_UINT：
    VkFormat depthFormat = device->findDepthFormat();

    // 创建深度图像和分配内存
    // - TILING_OPTIMAL: GPU 优化布局，性能最佳
    // - USAGE_DEPTH_STENCIL_ATTACHMENT_BIT: 用作深度/模板附件
    // - DEVICE_LOCAL_BIT: 存储在GPU 专用内存与
 ...
 * 渲染通道定义了需要什么类型的附件"，帧缓冲则提供实际的图像视图。
 * 
 * 每个帧缓冲包含：
 * - 一个颜色附件（来自交换链图像）
 * - 一个深度附件（共享同一个深度缓冲）
...
void VulkanSwapChain::createFramebuffers() {
    ...
    for (size_t i = 0; i < swapChainImageViews.size(); i++) {
        ...
        std::array<VkImageView, 2> attachments = {
            swapChainImageViews[i],  // 颜色附件：每帧不合
        ...
        framebufferInfo.layers = 1;                      // 层数（非数组纹理与：
